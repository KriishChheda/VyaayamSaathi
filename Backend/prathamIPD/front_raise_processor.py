import cv2
import time
from run_front_raise import FrontRaiseArmTracker, calculate_angle, draw_form_status, draw_banner, mp_pose, mp_drawing, mp_drawing_styles
from thresholds import get_thresholds_beginner

class ProcessFrameFrontRaise:
    def __init__(self, flip_frame=True):
        self.flip_frame = flip_frame
        self.left_arm = FrontRaiseArmTracker("LEFT")
        self.right_arm = FrontRaiseArmTracker("RIGHT")
        self.been_at_top = False
        self.press_counter = 0 # Used generically to count reps
    ...
    def process(self, frame, pose):
        ...
        try:
            if results.pose_landmarks:
                ...
                # ─── LEFT ARM (Actually Right due to Mirroring) ───
                ...
                if l_vis:
                    ...
                    if issue:
                        form_issues.append((issue, (0, 0, 200)))
                ...
                # ─── RIGHT ARM (Actually Left due to Mirroring) ───
                ...
                if l_vis and r_vis:
                    ...
                    if abs(self.left_arm.upper_arm_deg - self.right_arm.upper_arm_deg) > 25 and (self.left_arm.upper_arm_deg > 30 or self.right_arm.upper_arm_deg > 30):
                        form_issues.append(("UNEVEN - RAISE BOTH ARMS TOGETHER", (0, 140, 255)))
        ...
        if results.pose_landmarks:
            ...
            if self.left_arm.upper_arm_deg > 30 or self.right_arm.upper_arm_deg > 30:
                is_active = True
        ...
        cv2.putText(image_bgr, 'RAISE', (15, 65),
